...
export async function spawnZombie(type, scene, position) {
    ...
    // Per-zombie state
    model.userData.hitCount = 0;
    model.userData.dyingTimer = 0;
    model.userData.dyingAction = null;
    model.userData.resuming = false;
    const zombie = {
        type,
        model,
        mixer,
...
export function checkZombieHits(scene, getProjectiles) {
    ...
    for (let i = zombiesCopy.length - 1; i >= 0; i--) {
        ...
        for (let j = projectiles.length - 1; j >= 0; j--) {
            ...
            const projRadius = 0.08;
            if (zombiePos.distanceTo(projPos) < zombieRadius + projRadius) {
                // DEBUG: Visual feedback
                zombie.model.material = new THREE.MeshStandardMaterial({ color: 0xff0000 });
                console.log(`Zombie hit! Type: ${zombie.type}, HitCount: ${zombie.model.userData.hitCount || 0}`);
